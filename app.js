angular
  ...
  .controller(
    "EpisodeListCtrl",
    function ($scope, $http, $location, $cacheFactory) {
      $scope.apiKey = $location.search().apikey; // Extract API key from URL
      $scope.showId = $location.search().showid; //57532; // Replace with your actual show ID
      ...
      if ($scope.apiKey && $scope.showId) {
        $scope.getEpisodesDetails($scope.showId);
      }
    }
  );
